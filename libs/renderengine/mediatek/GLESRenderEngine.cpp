...
void GLESRenderEngine::setupLayerDither(bool val) {
    mState.isDither = val;
}

void GLESRenderEngine::setupLayerTexturingByDitherTable(const Texture& texture) {
    Texture localTexture = texture;
    localTexture.init(Texture::TEXTURE_2D, mDitherTableTexName);

    glBindTexture(GL_TEXTURE_2D, mDitherTableTexName);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);

    mState.texture = localTexture;
    mState.textureEnabled = true;
}

void GLESRenderEngine::initDitherTable() {
    unsigned char ditherTable[64 * 64] = {0};
    FILE* fp = fopen("/system/etc/HBMNoiseTable","rb");
    if (fp) {
        fread(ditherTable, 64 * 64, 1, fp);
        fclose(fp);
    } else {
        ALOGE("open file /system/etc/HBMNoiseTable failed (%s)", strerror(errno));
    }
    glGenTextures(1, &mDitherTableTexName);
    glBindTexture(GL_TEXTURE_2D, mDitherTableTexName);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, 64, 64, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, ditherTable);
}

void GLESRenderEngine::initEGLVendor()
{
    const char* name = (const char*)glGetString(GL_VENDOR);
    if (strcmp(name, "Imagination Technologies") == 0)
        mVendor = Description::EGLVendor::IMG;
    else if (strcmp(name, "ARM") == 0)
        mVendor = Description::EGLVendor::ARM;
    else
        mVendor = Description::EGLVendor::DEFAULT;
}

void GLESRenderEngine::setupEGLVendor() {
    mState.vendor = mVendor;
}
#endif
